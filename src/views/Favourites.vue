      ...
      <div v-for="like in viewFavourites" class="product">
        ...
        <div class="product-description">
          ...
          <div>
            ...
            <p class="overflow">{{ `${like.price}руб` }}</p>
          </div>
          <button v-if="!like.inCart" @click="$store.dispatch('addInCart', like.id)" class="btn">Добавить в корзину</button>
          <button disabled v-else-if="like.inCart" class="btn btn-disabled">Товар в корзине!</button>
        </div>
      </div>
    </div>
  </div>
